...
\begin{definition}
  设 $M$ 是一个 $n$ 维光滑流形, $M$ 上一个黎曼度量就是 $M$ 上一个二阶对称的、正定的光滑协变张量场,
...
\begin{proof}
  ...
  $\D_{\frac{\partial}{\partial x^j}}\frac{\partial}{\partial x^i} = \Gamma_{ij}\frac{\partial}{\partial x^k}$,
...
\begin{equation}\label{eq:1.4.1}
  \frac{\diff X^k}{\diff t} + \Gamma_{ji}^k \frac{\diff x^i}{\diff t}X^j(t)=0,\quad k=1,\cdots,n.
\end{equation}
...
\begin{proof}
  ...
  由于 $X_i(b)=P_a^t(v_i)$ 是方程组~\eqref{eq:1.4.1} 的解, 故
...
\begin{remark}
  ...
  其逆为 $(P_0^t)^{-1}\colon M_{\gamma(t)}\to M_{\gamma(a)}$, 则
  \begin{align*}
    \D_v X
  ...
  \end{align*}
  \begin{proof}
    由于 $P_0^t$ 是平移同构, 故取 $M_{\gamma(0)}$ 的基底 $e_1,\cdots,e_n$,
    把它们沿 $\gamma(t)$ 平行移动后得到 $M_{\gamma(t)}$ 的基底
    $e_1(t),\cdots,e_n(t)$, 即 $e_i(t)=P_0^t(e_i)$. 设
    \[X(\gamma(t)) = \sum_{i=1}^n X^i(t)e_i(t),\]
    则
    \[\D_v X = \D_{\gamma'(t)} X|_x = \left.\frac{\diff X^i(t)}{\diff t}\right|_{t=0} e_i.\]
    另一方面,
    \[(P_0^t)^{-1}\bigl(X(\gamma(t))\bigr) 
    = (P_0^t)^{-1} \biggl(\sum_{i=1}^n X^i(t)e_i(t)\biggr) = \sum_{i=1}^n X^i(t)e_i.\]
    故
    \begin{align*}
      \left.\frac{\diff}{\diff t}\right|_{t=0}\Bigl[(P_0^t)^{-1} \bigl(X(\gamma(t))\bigr)\Bigr]
      & = \left.\frac{\diff}{\diff t}\right|_{t=0} \sum_{i=1}^n X^i(t)e_i \\
